...
if (!argv.auth && !argv.cookie) {
    console.error("[!] Missing authentication token. Please refer to: github.com/rohitaryal/imageFX-api");
    process.exit(1);
}
...
const resp = await fx.generateImage({
    prompt: argv.prompt,
    count: argv.count,
    aspectRatio: argv.ratio || "IMAGE_ASPECT_RATIO_LANDSCAPE",
    model: argv.model || "IMAGEN_4",
    seed: argv.seed || 0,
});
if (resp.Err || !resp.Ok) {
